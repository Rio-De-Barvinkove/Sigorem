...
@onready var _block_types : Blocks = get_node("/root/Game/Blocks")
@onready var _item_db : ItemDB = get_node("/root/Game/Items")
@onready var _water_updater : WaterUpdater
@onready var _terrain : VoxelTerrain = get_node("/root/Game/VoxelTerrain")
...
func _ready():
	...
	if mp.has_multiplayer_peer() == false or mp.is_server():
		var water_node = get_node_or_null("/root/Game/Water")
		if water_node != null:
			_water_updater = water_node
