...
- [x] **Генерація: критичні регресії (листопад 2025)** ✅ **ВИПРАВЛЕНО 13.11.2025**
  - [x] Початкове завантаження світу займає ~5 хв, раніше стартував за секунди. → ВИРІШЕНО (вимкнено use_optimization)
  - [x] Під час швидкого руху до межі активних чанків відбуваються фризи; chunking втратила плавність. → ПОКРАЩЕНО (rate limiting)
  - [x] Висота рельєфу обмежена ≈5 блоками: поверхня, одна «земля» і відразу печера → падіння в порожнечу. → ВИРІШЕНО (height_amplitude=32, enable_caves=true)
  - [x] На поверхні видно різкі стики чанків; межі читаються як квадратна сітка. → ВИПРАВЛЕНО (noise-based smoothing)
  - [x] Початкова «safe zone»: гарантовано плоский спавн без блоків у голові, нормальна колізійна валідація. → РЕАЛІЗОВАНО (StartingAreaGenerator)
  - [x] Перевірити/переписати OptimizationManager: нині лише гальмує, треба виміряти й усунути деградацію. → ВИМІРЕНО і ВИМКНЕНО (use_optimization=false)
  - [x] Інструментарій огляду: польотний режим + регульована швидкість, X-ray/підсвітка печер, тумблер освітлення. → ЧАСТКОВО (польоти і швидкість реалізовані)
  - [ ] Creative/build mode: динамічне масштабування області будівництва/копання, режим камери від першої особи. → TODO
  - [x] Інтегрувати корисні команди у внутрішню консоль (телепорт, перегенерація чанків, профілювання тощо). → РЕАЛІЗОВАНО (ConsoleCommands)
