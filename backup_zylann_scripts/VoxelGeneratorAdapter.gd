...
func _generate_block(out_buffer: VoxelBuffer, origin: Vector3i, lod: int):
	...
	# Quick fill for blocks far below terrain - all air (smooth terrain handles underground)
	if block_max_y < 0:
		out_buffer.fill(BLOCK_AIR, VoxelBuffer.CHANNEL_TYPE)
		return
...
func _get_block_type(world_x: int, world_y: int, world_z: int, surface_height: int) -> int:
	# For mixed terrain: only generate blocks on/above surface (1-2 layers)
	# This creates blocky overlay on smooth terrain
	
	# Only generate blocks near surface (1-2 blocks thick)
	var surface_thickness := 2  # How many block layers on surface
	
	if world_y > surface_height + surface_thickness:
		return BLOCK_AIR  # Too high
	
	if world_y < surface_height:
		return BLOCK_AIR  # Below surface (smooth terrain handles this)
	
	# On surface or 1-2 blocks above
	var offset := world_y - surface_height
	if offset == 0:
		return BLOCK_GRASS  # Top layer
	elif offset == 1:
		return BLOCK_DIRT  # Second layer
	else:
		return BLOCK_AIR  # Too high
